...
- **端侧建议** — 通过内置 MNN 模型（Qwen3.5-0.8B）**完全离线**生成下一步建议，可选配**云端 LLM 服务**以获得更高质量。模型随 APK 打包并在首次使用时自动解压；未打包权重的构建会改为从 **ModelScope**（国内访问快）逐个文件下载，并对每个文件做 SHA-256 校验
